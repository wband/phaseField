...
void variableAttributeLoader::loadVariableAttributes(){
    ...
    std::string dep_valmudndt = string_valn+string_valmu+string_valdndt+string_gradmu;
    dep_valn.pop_back();
    dep_valmudndt.pop_back();
    string_gradmu.pop_back();
    string_gradn.pop_back();
    std::vector<bool> nucleation{false, true, true};
    for (unsigned int var_index=0; var_index<n_phases; var_index++){
        set_dependencies_value_term_RHS(var_index, dep_valn);
        set_dependencies_gradient_term_RHS(var_index, "");
        set_allowed_to_nucleate                 (var_index, nucleation[var_index]);
        set_need_value_nucleation               (var_index, nucleation[var_index]);
    }
