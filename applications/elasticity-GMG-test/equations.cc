  ...
  [[maybe_unused]] const VectorizedArray<double> element_volume) const
{
  ...
  scalarvalueType r = q_point_loc.norm();
  scalarvalueType phi{0.0};
  scalarvalueType c{0.0};
          ...
          if (r[i] < r_phi_in)
            {
              ...
              phi[i] += 0.5*std::sin(pi/2.0 -
                (r[i]-r_phi_in)/(r_phi_out-r_phi_in))+0.5;
            }
        }
      if (r[i] > r_c_in)
        {
          c[i] = (r[i]-r_c_in)*(r[i]-r_c_in)/(r_c_smooth + (r[i]-r_c_in))*
              (r_c_smooth+r_c_out-r_c_in)/((r_c_out-r_c_in)*(r_c_out-r_c_in));
        }
    }
      ...
      for (unsigned int j = 0; j < dim; j++)
        {
          ...
          if (i == j)
            {
              E[i][j] -= vegard_param*c;
            }
        }
    }

  // compute stress tensor
  computeStress<dim>(CIJ*phi, E, S);
