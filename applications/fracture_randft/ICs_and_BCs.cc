...
void customPDE<dim,degree>::setInitialCondition(const dealii::Point<dim> &p, const unsigned int index, double & scalar_IC, dealii::Vector<double> & vector_IC, double* data){
    // ---------------------------------------------------------------------
	...
	//}
      ...
      if (index > 2){
          scalar_IC = Utilities::generate_normal_random_number(1.0, Esigma);
	  scalar_IC = std::max(scalar_IC,Emin);
        //  scalar_IC = 1.0;
      }
